...
class InsuranceAPI:
    ...
    def fetch_unlabeled_data(self, db_name):
        ...
        try:
            query = """
            SELECT CustSsn, Age, BMI, Weight, Height, SmokingHabit, 
    ...
    def get_random_undetermined_client(self, db_name):
        ...
        Returns only the CustSsn of the random client.
        """
        self.connect(db_name)
        try:
            ...
            print(f"Random client chosen: CustSsn={random_client[0]}")
            return random_client[0]  # Return CustSsn
        except mysql.connector.Error as err:
            print(f"Error: {err}")
            return None
        finally:
    ...
    def recommend_products(self, client_ssn, db_name):
        ...
        try:
            ...
            recommendation_message = f"Hello {full_name}! The recommendation for you is the product '{series_name}' under the plan '{plan_name}'."
