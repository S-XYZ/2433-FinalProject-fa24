...
class ClientAPI:
    ...
    def generate_health_metrics_df(self, customer_df):
        ...
        for cust_id, row in customer_df.iterrows():
            ...
            health_metrics.append({
                ...
                "Mental": self._generate_health_score(row["HasChronicDisease"]),
                "Physical":self._generate_health_score(row["HasChronicDisease"]),
                "Happiness":self._generate_health_score(row["HasChronicDisease"])
            })
    ...
    def _generate_health_score(self, has_chronic_disease):
        """Generate mental/physical/happiness score based on chronic disease status."""
        if has_chronic_disease is True:
            return random.randint(0, 100)  # Good/positive score
        elif has_chronic_disease is False:
            return random.randint(-100, 0)  # Bad/negative score
        else:
            return random.randint(-100, 100)  # Undetermined
